...
case_pop_race <- m %>% summarize(black = sum(black),
                white = sum(white),
  ...
  mutate(race_pop_percent = c(0.059, 0.782, 0.083)) %>%
...
sum_count_mid <- left_join(mid_dis, total_pop,
                           by = "district_court") %>%
  mutate(district_court = as_factor(district_court)) %>%
  mutate(total = count + sum,
         count = count / total,
         sum = sum / total) %>%
  select(-total) 

sum_count <- sum_count_mid %>%
  arrange(desc(sum)) %>%
  pivot_longer(cols = c(count, sum),
                                 names_to = "ratio",
                                 values_to = "number") %>%
  mutate(order = rep(seq(1, 12, by = 1), each = 2)) 


saveRDS(sum_count, file = "shiny-app/data/sum_count.rds")
...
saveRDS(sum_count_mid, file = "shiny-app/data/sum_count_mid.rds")

sum_count_table <- sum_count_mid %>% 
...
pop_race_percent <- left_join(pop_race_total, 
  ...
  mutate(race = case_when(race == "black_percent" ~ "black",
                               race == "white_percent" ~ "white",
                               race == "hispanic_percent" ~ "hispanic",
                               TRUE ~ "Other"))
...
mid_dis
m
m2 <- left_join(m, mid_dis, by = "district_court") %>%
  rename(sum = count) %>%
  mutate(mp_black = black/sum,
         mp_white = white/sum,
         mp_hispanic = hispanic/sum) %>%
  select(district_court, mp_black, mp_white, mp_hispanic) %>%
  pivot_longer(cols = -district_court, 
               names_to = "case_race", 
               values_to = "case_percent") %>%
  mutate(case_race = case_when(case_race == "mp_black" ~ "black",
                               case_race == "mp_white" ~ "white",
                               case_race == "mp_hispanic" ~ "hispanic",
                               TRUE ~ "Other")) %>%
...
#### Investigate each district court ####
 left_join(pop_race_percent, m2, 
                           by = c("race", "district_court")) %>%
  filter(district_court == "Ayer District Court") %>%
  pivot_longer(cols = c(pop_percent, case_percent),
              names_to = "type",
              values_to = "value") %>%
   ggplot(aes(x = factor(race, levels = c("white", "black", "hispanic")), 
              y = value, fill = forcats::fct_rev(type))) +
   geom_col(position = "dodge") +
   theme_minimal() +
  scale_fill_manual(name = "Race",
                    values = c("salmon", "dodgerblue"),
                    breaks = c("pop_percent", "case_percent"),
                    labels = c("% Total population", "% Total cases")) +
  labs(title = "Cases by Population (by Race)", 
       x = "",
       y = "") +
...
total_sum_count <- left_join(mid_dis, total_pop, by = "district_court") %>%
  pivot_longer(cols = -district_court, names_to = "type", values_to = "value")
...
m3 <- middlesex_district %>% 
  ...
  select(case_number, court_location, filing_offense_diff, dispo_filing_diff) %>%
...
m3 %>%
  filter(court_location == "Lowell District Court") %>%
...
saveRDS(m3, file = "shiny-app/data/m3.rds")

m3a <- middlesex_district %>% 
  mutate(court_location = case_when(
                   court_location == "AYE" ~ "Ayer District Court",
                   court_location == "CAM" ~ "Cambridge District Court",
                   court_location == "CON" ~ "Concord District Court",
...
m3a %>% pivot_longer(cols = c(filing_offense_diff, dispo_filing_diff),
               names_to = "stage",
               values_to = "length") %>%
  filter(court_location == "Lowell District Court") %>%
...
m3a %>% 
  filter(defendant_race %in% c("Black", "White")) %>%
  group_by(defendant_race) %>%
  summarize(median_1 = median(filing_offense_diff),
            max_1 = max(filing_offense_diff),
            min_1 = min(filing_offense_diff)) %>%
  pivot_longer(cols = ends_with("_1"), names_to = "filing_offense", values_to = "value")

# fill = forcats::fct_rev(stage))
# median_2 = median(dispo_filing_diff)
# max_2 = max(dispo_filing_diff)
# min_2 = min(dispo_filing_diff)
```
...
# filter to only district courts
    ...
    select(-criminal_count_number, -charge_crime_code, -charge_crime_description,
           charge_crime_type, -defendant_gender, -name_of_judge, -disposition_code,
           -disposition_description, -sentence_recommended, -sentence_type, 
...
m4 %>%
  filter(court_location == "Ayer District Court") %>%
  ggplot(aes(x = date_of_filing, y = filing_offense_diff), color = "blue") +
    geom_line() +
    geom_line(data = m4, aes(x = disposition_date, y = dispo_filing_diff), color = "red") 
