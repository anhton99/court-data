...
# Racial population as % population served  
pop_race_percent <- left_join(pop_race_total, 
  ...
  select(district_court, black_percent, white_percent)
...
total_sum_count <- left_join(middlesex_district, total_pop, 
                             by = "district_court") %>%
  pivot_longer(cols = -district_court, 
...
```{r leaflet}
join_pop
map <- left_join(join_pop, pop_race_percent, by = "district_court")


leaflet(map) %>%
  addTiles() %>%
  addPolygons(fillColor = topo.colors(10, alpha = 0.8))

middlemap <- st_read("raw_data/tl_2017_25017_roads.shp")
...
race_count <- count %>% 
  filter(defendant_race %in% c("Black", "White")) %>%
  ggplot(aes(x = sum_count)) +
  geom_histogram(bins = 100) + 
  geom_vline(aes(xintercept = median(sum_count), color = "median")) +
  facet_wrap(~defendant_race)
  # 59,983 rows 
  
unique(count$sum_count)

middlesex_2 %>% filter(case_number == "14-01-479875")

# see a case with > 1000 counts 
middlesex_2 %>% 
...
unique(middlesex_2$charge_crime_type)

count %>% 
  filter(defendant_race %in% c("Black", "White")) %>%
  filter(court_location == "Newton District Court") %>%
  ggplot(aes(x = sum_count)) +
...
# filter to only district courts
    ...
    filter(!court_location %in% c("CAMJU", "FRAJU", "LOWJU", "WALJU",
                                  "LSUP", "MSUP", 
                                  "NA", "OTH")) %>% 
  select(case_number, court_location, disposition_description, defendant_race) 
